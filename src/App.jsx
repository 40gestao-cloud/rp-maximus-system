...
const CHECKPOINTS = [
  ...
  { id:"break",  label:"Intervalo", time:"09:00", icon:"⏸️" },
  { id:"return", label:"Retorno",   time:"09:20", icon:"🔄" },
  { id:"exit",   label:"Saída",     time:"11:20", icon:"🏁" },
];
const CP_ORDER = ["entry","break","return","exit"];
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      const [e,r,dc] = await Promise.all([load(KEYS.EMPLOYEES),load(KEYS.RECORDS),load(KEYS.DAILY_CODE)]);
      if(e)  setEmployees(e);
      if(r)  setRecords(r);
  ...
  const saveEmp = useCallback(async d=>{setEmployees(d);await save(KEYS.EMPLOYEES,d);},[]);
  const saveRec = useCallback(async d=>{setRecords(d);  await save(KEYS.RECORDS,d);  },[]);
  const saveDC  = useCallback(async d=>{setDailyCode(d);await save(KEYS.DAILY_CODE,d);},[]);
  ...
  return(
    <div className="login-root">
      <div className="login-panel">
        <div className="brand-name">GP MAXIMUS</div>
        <div className="brand-sub">Sistema de Gestão de Pessoas</div>
    ...
    <div className="app">
      <nav className="sidebar">
        <div className="sb-logo">
          <div className="brand-name">GP MAXIMUS</div>
          <div className="brand-sub" style={{fontSize:9,letterSpacing:3,textTransform:"uppercase",color:"var(--muted)",marginTop:4}}>Sistema de GP</div>
        </div>
      ...
      <main className="main">
        ...
        {page==="attendance" && <AdminAttendance employees={employees} records={records}/>}
      ...
      <div className="card">
        ...
        {todayRecs.length===0
          ...
          : <div className="tbl-wrap">
              <table>
                <thead><tr><th>Colaborador</th><th>Setor</th><th>Entrada</th><th>Intervalo</th><th>Retorno</th><th>Saída</th></tr></thead>
                <tbody>
                  {todayRecs.map(r=>{
                    ...
                    return(
                      <tr key={r.empId}>
                        ...
                        <td style={{color:"var(--gold)",fontFamily:"var(--mono)",fontSize:12}}>{r.break||"—"}</td>
                        <td style={{color:"var(--blue)",fontFamily:"var(--mono)",fontSize:12}}>{r.return||"—"}</td>
...
function AdminEmployees({employees,saveEmp}){
  ...
  const handleSave=async()=>{
    ...
    let upd=editId?employees.map(e=>e.id===editId?{...e,...form}:e):[...employees,{id:Date.now().toString(),...form}];
    await saveEmp(upd);
    setModal(false);
  ...
  const handleDel=async id=>{await saveEmp(employees.filter(e=>e.id!==id));};
...
/* ── Attendance ── */
function AdminAttendance({employees,records}){
  ...
  return(
    <div>
      ...
      <div className="card">
        <div className="tbl-wrap">
          <table>
            <thead><tr><th>Nome</th><th>Setor</th><th>Status</th><th>Entrada</th><th>Intervalo</th><th>Retorno</th><th>Saída</th></tr></thead>
            <tbody>
              {filtered.map(e=>{
                ...
                return(
                  <tr key={e.id}>
                    ...
                    <td style={{color:"var(--gold)",fontFamily:"var(--mono)",fontSize:12}}>{rec?.break||"—"}</td>
                    <td style={{color:"var(--blue)",fontFamily:"var(--mono)",fontSize:12}}>{rec?.return||"—"}</td>
...
/* ═══════════════════════════════════════════════════════════════
   EMPLOYEE AREA — Tela do Aluno/Colaborador
═══════════════════════════════════════════════════════════════ */
function EmployeeArea({session,onLogout,records,dailyCode,saveRec}){
  ...
  const handleCheckin = async()=>{
    ...
    if(!todayCode){
      setMsg({t:"err",m:"Nenhum código disponível hoje. Solicite ao professor."});
      setTimeout(()=>setMsg(null),4000); return;
    }
    if(code.toUpperCase()!==todayCode){
      setMsg({t:"err",m:"Código incorreto. Verifique com o professor."});
            ...
            {selCP&&(
              <div className="code-input-area pop">
                ...
                <div className="txt-muted mt2 mb3">
                  Digite o código de 6 letras exibido pelo professor
                </div>
                <button className="btn btn-gold btn-full" onClick={handleCheckin}>
                  CONFIRMAR — {CHECKPOINTS.find(c=>c.id===selCP)?.label.toUpperCase()}
                </button>
              </div>
            )}
          </>
